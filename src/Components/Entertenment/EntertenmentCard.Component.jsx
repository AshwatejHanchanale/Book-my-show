...
const EntertenmentCardSlider = () => {
    const EntertenmentImage = [
    ...
    return (
        <>
            <Slider {...settings}>
                {EntertenmentImage.map((image, index) => (
                    <EntertenmentCard src={image} key={index} />
                ))}
            </Slider>
        </>
    );
};
export default EntertenmentCardSlider;